...
_,exps = calcTheta_poly_2D(t_out[1:-1],X_clean[1:-1,0],X_clean[1:-1,1],order=2)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        ...
        w_ident = SINDy_LV(t_out,X_noisy,params_regression)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_bump_LV(t_out,X_noisy,20,20,params_regression)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_FFT_PSD_2D(t_out,X_noisy,N_freq,params_regression)
