...
gridDensity = 100
t_span = [0,t_sim]
t_out,X_clean,XD_out = simulateVDP(X0,t_sim,gridDensity,mu)

# true coefficient matrix
Theta,exps = calcTheta_poly_2D(t_out[1:-1],X_clean[1:-1,0],X_clean[1:-1,1],order=3)
w_true = trueCoeffMatrix_VDP(exps,mu)
w_true
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        ...
        w_ident = SINDy_VDP(t_out,X_noisy,params_regression,polyOrder=3)
...
params_regression = dict()
params_regression["method"] = "ridge"
params_regression["lambda_sparse"] = 0.2
params_regression["lambda_ridge"] = 0.0
params_regression["N_loops"] = 100
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_bump_VDP(t_out,X_noisy,20,20,params_regression,polyOrder=3)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_FFT_PSD_2D(t_out,X_noisy,N_freq,params_regression,polyOrder=3)
