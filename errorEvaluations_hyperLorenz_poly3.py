...
Theta,exps = calcTheta_poly_4D(t_out[1:-1],X_clean[1:-1,0],X_clean[1:-1,1],X_clean[1:-1,2],X_clean[1:-1,3],order=3)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        ...
        w_ident = SINDy_4D(t_out,X_noisy,params_regression,polyOrder=3)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_bump_4D(t_out,X_noisy,20,20,params_regression,polyOrder=3)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_Fourier_FFT_4D(t_out,X_noisy,N_freq,params_regression,polyOrder=3)
...
savemat("hyperLorenz_poly3.mat",data)