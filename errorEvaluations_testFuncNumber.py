...
arr_testFuncNum_bump = np.array([9,16,36,49,100,225,400])   # array of number of test functions for bump weak SINDy
arr_testFuncNum_Fourier = np.array([10,20,30,60,100,200,400])   # array of number of test functions for bump weak SINDy
...
# evaluate error with different numbers of test functions
arr_relError_mean = np.zeros((1,len(arr_testFuncNum_bump))) # allocate array of mean relative error norms for each noise level
arr_TPR_mean = np.zeros((1,len(arr_testFuncNum_bump))) # allocate array of mean TPR for each noise level
for i in range(0,len(arr_testFuncNum_bump)):
    N_p = int(np.sqrt(arr_testFuncNum_bump[i]))
    N_q = int(np.sqrt(arr_testFuncNum_bump[i]))
    ...
    for j in range(0,N_noise):
        ...
        w_ident = WSINDy_bump_Lorenz(t_out,X_noisy,N_p,N_q,params_regression)
...
# evaluate error with different numbers of test functions
arr_relError_mean = np.zeros((1,len(arr_testFuncNum_Fourier))) # allocate array of mean relative error norms for each noise level
arr_TPR_mean = np.zeros((1,len(arr_testFuncNum_Fourier))) # allocate array of mean TPR for each noise level
for i in range(0,len(arr_testFuncNum_Fourier)):
    N_freq = int(arr_testFuncNum_Fourier[i]/2)
    arr_relError = np.zeros((1,N_noise))   # array of relative error norms for N noises with current noise level
    arr_TPR = np.zeros((1,N_noise)) # array of TPRs for N noises with current noise level
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
...
# ---log data---
data = dict()
data["arr_testFuncNum_bump"] = arr_testFuncNum_bump
