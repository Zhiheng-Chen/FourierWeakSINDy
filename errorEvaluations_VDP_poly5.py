...
Theta,exps = calcTheta_poly_2D(t_out[1:-1],X_clean[1:-1,0],X_clean[1:-1,1])
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        ...
        w_ident = SINDy_VDP(t_out,X_noisy,params_regression,polyOrder=5)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_bump_VDP(t_out,X_noisy,20,20,params_regression,polyOrder=5)
...
params_regression = dict()
params_regression["method"] = "ridge"
params_regression["lambda_sparse"] = 0.6
params_regression["lambda_ridge"] = 0.001
params_regression["N_loops"] = 100
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_Fourier_FFT_VDP(t_out,X_noisy,N_freq,params_regression,polyOrder=5)
