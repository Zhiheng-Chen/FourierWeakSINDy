...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        ...
        w_ident = SINDy_VDP(t_out,X_noisy,params_regression)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_bump_VDP(t_out,X_noisy,20,20,params_regression)
...
for i in range(0,len(arr_sig_NR)):
    for j in range(0,N_noise):
        X_noisy = arr_X_noisy[:,:,i,j]
        w_ident = WSINDy_Fourier_FFT_VDP(t_out,X_noisy,N_freq,params_regression)
...
savemat("results_noiseLevelAndCoeffs_VDP.mat",data)